...
public class Accountc implements Serializable {
  ...
  static Logger logger = Logger.getLogger(Accountc.class);
  ...
  private static HashMap<Integer, Accountc> accounts = new HashMap<>();
...
/*
  //Main Constructor
  public Accountc(User user) throws Exception {
    super();
    Menu mini= new Menu();
    String name, input;
    double bbalance=0;
    ...
    for (Map.Entry<Integer, Accountc> set :
      accounts.entrySet()) {
  ...
  public Transaction Withdrew(User user) throws IOException {
    ...
    double bbalance=0;
    int counter=1;
    Accountc accounts1=user.getAccounts();
    System.out.println("\t\t\tAccount\nOption\t\tName\t\tNumber\t\tBalance");
    for (Map.Entry<Integer, Accountc> set : user.getAccountsh().entrySet()) {
  ...
  public void CreateAccount(User user) throws Exception {
    ...
    Accountc account = new Accountc(user);
  ...
  public HashMap<Integer, Accountc> getAccounts() {
